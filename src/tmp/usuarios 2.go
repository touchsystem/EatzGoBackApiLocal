...
func (repositorio Usuarios) Criar(usuario modelos.Usuario) (uint64, error) {
	statement, erro := repositorio.db.Prepare(
		"insert into usuarios (nome, nick, email, senha, cdemp) values(?, ?, ?, ?, ?)",
	)
	if erro != nil {
		return 0, erro
	}
	...
	resultado, erro := statement.Exec(usuario.Nome, usuario.Nick, usuario.Email, usuario.Senha, usuario.CDEMP)
	if erro != nil {
		return 0, erro
	}
...
func (repositorio Usuarios) Buscar(nomeOuNick string) ([]modelos.Usuario, error) {
	...
	linhas, erro := repositorio.db.Query(
		"select id, nome, nick, email, criadoEm, CDEMP from usuarios where nome LIKE ? or nick LIKE ?",
...
func (repositorio Usuarios) BuscarPorEmail(email string) (modelos.Usuario, error) {
	linha, erro := repositorio.db.Query("select id, senha, CDEMP from usuarios where email = ?", email)
	if erro != nil {
		return modelos.Usuario{}, erro
	}
	defer linha.Close()
	...
	if linha.Next() {
		if erro = linha.Scan(&usuario.ID, &usuario.Senha, &usuario.CDEMP); erro != nil {
			return modelos.Usuario{}, erro
		}
	}
